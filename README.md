...
```sh
make docker-build docker-push IMG=<some-registry>/namespace-resizer:tag
```
...
```sh
make deploy IMG=<some-registry>/namespace-resizer:tag
```
...
```sh
make build-installer IMG=<some-registry>/namespace-resizer:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/namespace-resizer/<tag or branch>/dist/install.yaml
```
