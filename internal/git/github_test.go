...
func TestCreatePR(t *testing.T) {
	...
	mux.HandleFunc("/repos/o/r/contents/managed-resources/cluster/default/quota.yaml", func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "GET" {
			// Return content
			fmt.Fprint(w, `{"content": "a2luZDogUmVzb3VyY2VRdW90YQptZXRhZGF0YToKICBuYW1lOiBteS1xdW90YQpzcGVjOgogIGhhcmQ6CiAgICByZXF1ZXN0cy5jcHU6IDE=", "encoding": "base64", "sha": "file-sha"}`)
		} else if r.Method == "PUT" {
			fmt.Fprint(w, `{"commit": {"sha": "new-sha"}}`)
		}
	}) // 7. Create PR
	mux.HandleFunc("/repos/o/r/pulls", func(w http.ResponseWriter, r *http.Request) {
		g.Expect(r.Method).To(Equal("POST"))
		fmt.Fprint(w, `{"number": 101, "state": "open"}`)
	})
...
func TestUpdatePR(t *testing.T) {
	...
	mux.HandleFunc("/repos/o/r/pulls/101", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprint(w, `{"head": {"ref": "resize/branch"}}`)
	})
